...
public class WorldGenBigOliveTree  extends WorldGenAbstractTree {
    ...
    List<WorldGenBigOliveTree.FoliageCoordinates> foliageCoords;

    public WorldGenBigOliveTree(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenBigOliveTree.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenBigOliveTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenBigOliveTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeaves()
    {
        for (WorldGenBigOliveTree.FoliageCoordinates WorldGenBigOliveTree$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(WorldGenBigOliveTree$foliagecoordinates);
        }
    }
    ...
    void generateTrunk()
    {
        ...
        Block block = ModBlocks.olivelog;
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenBigOliveTree.FoliageCoordinates WorldGenBigOliveTree$foliagecoordinates : this.foliageCoords)
        {
            int i = WorldGenBigOliveTree$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenBigOliveTree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, WorldGenBigOliveTree$foliagecoordinates, ModBlocks.olivelog);
            }
        }
    }
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, ((OliveSapling)ModBlocks.olivesapling));
