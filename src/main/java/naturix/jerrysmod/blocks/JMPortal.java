...
public class JMPortal {
	...
	public static DimensionType dtype = (ModDimensions.jerryDimensionType);
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? Blocks.SLIME_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.SLIME_BLOCK.getDefaultState() : iblockstate,
								2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block {
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.SLIME_BLOCK) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.SLIME_BLOCK) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.SLIME_BLOCK || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.SLIME_BLOCK || getBlock(par1World, par2 + b0, par3,
										par4 + b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		...
		protected Biome[] allowedBiomes = {ModBiomes.SlimeBiome,};
